...
async function createTemplate() {
  try {
    ...
    // 3. 创建任务集与任务的关联关系（设置正确的优先级和排序）
    console.log('\n3. 创建任务集关联关系（设置优先级）...');
    
    // 听课任务集关联 - 根据天数设置不同优先级
    for (let i = 1; i <= 7; i++) {
      const taskId = tasks[`听课-${i}`];
      const priority = i <= 4 ? 1 : 3; // Day1-4为优先度I，Day5-7为优先度III
    ...
    console.log(`   ✓ 听课任务集: 7个关联（Day1-4: 优先度I, Day5-7: 优先度III）`);
    ...
    // 计算limit_hour（以日期为key的对象格式）
    const limitHour = {
      "1": 570,  // 听课180 + 看直播180 + 看书120 + 做题60 + 单词30 = 570
      "2": 570,
      "3": 570,
      "4": 570,
      "5": 570,
      "6": 570,
      "7": 570,
      "8": 390,  // 作文60 + 看直播180 + 健身60 + 做题60 + 单词30 = 390
      "9": 390,  // 作文60 + 看直播180 + 跳舞60 + 做题60 + 单词30 = 390
      "10": 360  // 作文60 + 看直播180 + 玩耍30 + 做题60 + 单词30 = 360
    };
    
    const totalTime = Object.values(limitHour).reduce((sum, val) => sum + val, 0);
    ...
    console.log(`   ✓ limit_hour格式: ${JSON.stringify(limitHour)}`);
    ...
    for (let day = 1; day <= 7; day++) {
      const listenPriority = day <= 4 ? 1 : 3; // Day1-4为优先度I，Day5-7为优先度III
    ...
    // 验证limit_hour格式
    const createdTemplate = await prisma.planTemplate.findUnique({
      where: { id: template.id },
      select: { limit_hour: true }
    });
    console.log(`\n   ✓ limit_hour格式验证:`);
    console.log(`      ${JSON.stringify(createdTemplate.limit_hour)}`);
    
    // 验证任务集关联关系的优先级
    console.log(`\n   ✓ 任务集关联关系优先级验证:`);
    const relations = await prisma.platformTaskGroupAndTaskRelation.findMany({
      where: {
        platform_task_group_id: { in: Object.values(groups) }
      },
      include: {
        platform_task: {
          select: { name: true }
    ...
    relations.forEach(rel => {
      const groupName = rel.platform_task_group.name;
      if (!groupedRelations[groupName]) {
        groupedRelations[groupName] = [];
      }
      groupedRelations[groupName].push({
        task: rel.platform_task.name,
        priority: rel.priority,
        group_sort: rel.group_sort
      });
    });
    
    Object.entries(groupedRelations).forEach(([groupName, items]) => {
      console.log(`      ${groupName}:`);
      items.slice(0, 3).forEach(item => {
        console.log(`        ${item.task}: 优先度${item.priority === 9999 ? '无' : item.priority}, 排序${item.group_sort}`);
      });
      if (items.length > 3) {
        console.log(`        ... 共${items.length}个任务`);
      }
    });
