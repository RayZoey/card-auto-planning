...
async function main() {
  ...
  for (const day of Object.keys(grouped).sort((a,b)=>Number(a)-Number(b))) {
    ...
    for (const item of grouped[Number(day)]) {
      ...
      console.log(`  ${item.day_sort}. ${item.task?.name} (${dur}min) - priority ${item.priority} task_id ${item.task?.id} group_sort ${item.group_sort}`);
    }
