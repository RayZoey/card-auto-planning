 ...
 * @LastEditTime: 2025-09-06 16:11:17
...
export class UserPlanController {
   ...
   * 手动触发自动规划 - 用于手动创建的计划
   */
  @Post(':id/trigger-planning')
  @UseGuards(JwtAuthGuard, RoleGuard('miniUser'))
  async triggerAutoPlanning(@Req() req, @Param('id') planId: number) {
    const userId = req.user.accountId;
    const res = await this.service.triggerAutoPlanning(planId, userId);
    return {                                     
      code: HttpStatus.OK,
      data: res,
      res: '自动规划触发成功',
  ...
  @Post(':id/update-plan')
  @UseGuards(JwtAuthGuard, RoleGuard('miniUser'))
  async updatePlan(@Req() req, @Param('id') planId: number) {
    const userId = req.user.accountId;
    const res = await this.service.updatePlan(planId, userId);
    return {                                     
      code: HttpStatus.OK,
      data: res,
      res: '规划更新成功',
   ...
   * 获取计划的每日任务概览
   */
  @Get(':id/daily-overview')
  @UseGuards(JwtAuthGuard, RoleGuard('miniUser'))
  async getPlanDailyOverview(
    @Req() req, 
    ...
    const userId = req.user.accountId;
    const res = await this.service.getPlanDailyOverview(planId, userId, startDate, endDate);
    return {                                     
      code: HttpStatus.OK,
      data: res,
      res: '成功',
