 ...
 * @LastEditTime: 2026-02-25 10:21:14
...
export class PlatformTaskGroupController {
  ...
  @UseGuards(JwtAuthGuard, RoleGuard('backUser'))
  async connectTask(@Param('id') id: number, @Body('task_list') taskList: [{id: number, group_sort: number}]) {
    const res = await this.service.connectTask(id, taskList);
    return {                                     
