 ...
 * @LastEditTime: 2025-08-18 15:58:25
...
import { PlanTemplateQuery } from './plan-template.query';

@Controller('plan-template')
export class PlanTemplateController {
  constructor(private readonly service: PlanTemplateService, private offsetCalculator: OffsetCalculator) {}

  @Get()
  @UseGuards(JwtAuthGuard, RoleGuard('backUser'))
  async list(@Req() request: Request, @Query() pagination: PaginationDto, @Query() queryDto: PlanTemplateQuery) {
    const offset = this.offsetCalculator.calculate(pagination.page, pagination.pageSize);
    const limit = pagination.pageSize;
    const queryCondition = new PlanTemplateQueryCondition();
  ...
  @Post()
  @UseGuards(JwtAuthGuard, RoleGuard('backUser'))
  async create(@Res() response: Response, @Body() createDto: PlanTemplateCreateDto) {
  ...
  @Put(':id')
  @UseGuards(JwtAuthGuard, RoleGuard('backUser'))
  async update(@Param('id') id: number, @Body() updateDto: PlanTemplateUpdateDto, @Res() response: Response) {
