...
import { AdminUserTaskUpdateDto } from './task.update.dto';
...
export class AdminUserTaskService {
  constructor(
    ...
    @Inject(forwardRef(() => AdminUserPlanService))
    private readonly planService: AdminUserPlanService,
  ) {}
  ...
  async create(userId: number, dto: AdminUserTaskCreateDto, needAutoPlan: boolean, needAutoFill: boolean) {
  ...
  async update(id: number, dto: AdminUserTaskUpdateDto, userId: number) {
  ...
  //  编辑用户任务基础信息（不涉及排序）
  async updateBaseInfo(id: number, dto: AdminUserTaskBaseUpdateDto, userId: number) {
    ...
    //  如果任务集id变更，提示报错
    if (dto.task_group_id != null && dto.task_group_id !== scheduler.task.task_group_id) {
      throw new Error('暂不支持编辑任务时修改所属任务集');
    }
